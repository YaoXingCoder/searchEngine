...
class WebPage {
  public:
    ...
    // 获取topK词, 到给定的容器中
    void calcTopK(std::vector<std::string> &wordsVec, std::unordered_set<std::string> &stopWordSet,
                  int k = TOPK_NUMBER);                                           // 获取文档的topk词集
    void calcTopKNoStop(std::vector<std::string> &wordsVec, int k = TOPK_NUMBER); // 获取文档的topk词集, 无停用词
  ...
  private:
    ...
    void parseContent(const std::string &content,
                      std::unordered_set<std::string> &stopWordSet); // 单词和词频, 去除停用词
  ...
  private:
    ...
    std::unordered_map<std::string, int> _wordsMap; // 保存每篇文档的所有词语和词频
